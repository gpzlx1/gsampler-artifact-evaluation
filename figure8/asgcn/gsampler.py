...
def asgcn_sampler(
    ...
    for K in fanouts:
        ...
        node_feats_u, node_feats_v = get_feature(features, subA.rows(),
                                                 subA.cols(), use_uva)
    ...
    for epoch in range(n_epoch):
        ...
        for it, seeds in enumerate(tqdm.tqdm(seedloader)):
            input_nodes, output_nodes, blocks = sampler(
                graph, seeds, fanouts, features, W, args.use_uva)
...
def train(dataset, args):
    ...
    compile_func = gs.jit.compile(func=asgcn_sampler,
                                  args=(m, seeds, fanouts, features, W,
                                        args.use_uva), must_compact=args.use_uva)
