...
grep -Eo 'avg\. elapsed: [0-9.]+ ms' output/gunrock.log | awk -F': ' 'BEGIN{OFS=","; print "baseline,dataset,execution time,algorithm time"}{sub(/ ms/,"",$2); if (NR==1) {printf "gunrock,lj,%s,sage\n", $2} else {printf "gunrock,pd,%s,sage\n", $2}}' > output/gunrock.csv
